...
// ---------- توقيت عُمان وتحليل الأمر العربي (نفس منطق parse() في التطبيق) ----------
...
// ---------- التحقق من هوية المستخدم عبر Firebase ----------
...
// ---------- استقبال أمر Siri: يُحفظ فورًا بقاعدة البيانات + إشعار تأكيد ----------
// هذا يغلق الفجوة اللي كانت تخلي حفظ الأمر الصوتي يعتمد على فتح التطبيق.
async function ingestCommand(req,env){
  const url=new URL(req.url);
  const key=url.searchParams.get('key')||'';
  const uid=await uidForKey(env,key);
  ...
  await env.DB.prepare(
    'INSERT INTO items (id,uid,text,type,title,event_at,created_at,source) VALUES (?,?,?,?,?,?,?,?)'
  ...
  if(parsed.eventAt&&parsed.reminders.length){
    for(const offset of parsed.reminders){
      ...
      if(notifyAt>now-5*60000){
        await env.DB.prepare(
          'INSERT INTO reminders (id,item_id,uid,notify_at,offset_minutes,sent) VALUES (?,?,?,?,?,0)'
        ).bind(randomId('rem'),id,uid,notifyAt,offset).run();
      }
    }
  }

  await sendPushForUid(env,uid,{title:'مُعين',body:'تم الحفظ: '+parsed.title,itemId:id,url:'./'});
  return json({ok:true,item:{id,...parsed}});
}

// ---------- مزامنة تذكيرات التطبيق العادية (غير الصادرة من Siri) ----------
// يُستدعى من firebase-adapter.js في كل syncItems() — بدون هذا، أي تذكير
// تسوّينه من داخل التطبيق نفسه ما يوصله Push وقت إغلاق التطبيق.
async function syncReminders(req,env){
  ...
  // نعيد بناء تذكيرات التطبيق (source='app') من الحالة الحالية بالكامل —
  // نفس أسلوب الاستبدال الكامل المستخدم أصلًا بمزامنة Firebase.
  await env.DB.prepare('DELETE FROM reminders WHERE uid=? AND sent=0 AND item_id IN (SELECT id FROM items WHERE uid=? AND source=?)').bind(user.uid,user.uid,'app').run();
  await env.DB.prepare('DELETE FROM items WHERE uid=? AND source=?').bind(user.uid,'app').run();

  for(const it of items){
    if(!it||!it.id||it.done||it.type==='idea')continue;
    if(!it.date||!it.time||!Array.isArray(it.reminders)||!it.reminders.length)continue;
    const eventAt=toOmanEpoch(it.date,it.time);
    if(!eventAt||eventAt<now)continue;

    await env.DB.prepare(
      'INSERT INTO items (id,uid,text,type,title,event_at,created_at,source) VALUES (?,?,?,?,?,?,?,?)'
    ).bind('app_'+String(it.id),user.uid,it.title||'',it.type||'task',it.title||'',eventAt,now,'app').run();

    for(const offset of it.reminders){
      const notifyAt=eventAt-Number(offset)*60000;
      if(notifyAt>now-5*60000){
        await env.DB.prepare(
          'INSERT INTO reminders (id,item_id,uid,notify_at,offset_minutes,sent) VALUES (?,?,?,?,?,0)'
        ).bind(randomId('rem'),'app_'+String(it.id),user.uid,notifyAt,Number(offset)).run();
      }
    }
  }
...
// ---------- قراءة عناصر Siri لدمجها بقائمة التطبيق (تُستدعى عند فتح التطبيق) ----------
...
async function listItems(req,env){
  ...
  const items=(rows.results||[]).map(r=>{
    ...
    return {
      id:r.id,type:r.type||'task',title:r.title||r.text||'',notes:'',
      date:dt.date,time:dt.time,priority:'normal',
      reminders:[...new Set(reminders)],done:false,
...
// ---------- اشتراك الإشعارات (Web Push) — الآن يتطلب تسجيل دخول ----------
// قبل كان أي شخص يقدر يسجّل أي جهاز بدون تحقق من الهوية إطلاقًا.
async function subscribe(req,env){
  ...
  const now=Date.now();
  await env.DB.prepare(
    `INSERT INTO subscriptions(endpoint,uid,p256dh,auth,created_at,updated_at)
     VALUES(?,?,?,?,?,?)
     ON CONFLICT(endpoint) DO UPDATE SET uid=excluded.uid,p256dh=excluded.p256dh,auth=excluded.auth,updated_at=excluded.updated_at`
...
// يرسل فقط لأجهزة المستخدم صاحب uid — قبل كان يبث لكل الأجهزة المشتركة بالتطبيق.
...
async function testPush(req,env){
  ...
  const user=await verifyFirebaseUser(req);
  if(!user)return json({ok:false,error:'unauthorized'},401);
  return json({ok:true,result:await sendPushForUid(env,user.uid,{title:'مُعين',body:'الإشعارات تعمل بنجاح.',url:'./'})});
}

async function processDue(env){
  const now=Date.now();
  const due=await env.DB.prepare(
    `SELECT r.id,r.item_id,r.offset_minutes,i.title,i.uid
  ...
  for(const r of due.results||[]){
    try{
      const result=await sendPushForUid(env,r.uid,{
        title:'مُعين',
        body:reminderBody(r.title,Number(r.offset_minutes)||0),
        itemId:r.item_id,
        url:'./'
      });
      if(result.ok>0){
