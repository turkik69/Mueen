...
function omanDateParts(d=new Date()){
  const p=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Muscat',year:'numeric',month:'2-digit',day:'2-digit',weekday:'short'})
...
async function ensureSchema(env){
  ...
  await env.DB.prepare('CREATE TABLE IF NOT EXISTS subscriptions (endpoint TEXT PRIMARY KEY,p256dh TEXT NOT NULL,auth TEXT NOT NULL,user_id TEXT,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL)').run();
  await env.DB.prepare('CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY,user_id TEXT,text TEXT NOT NULL,type TEXT NOT NULL,title TEXT NOT NULL,event_at INTEGER,created_at INTEGER NOT NULL,source TEXT NOT NULL DEFAULT \'siri\')').run();
  ...
  if(!(await hasColumn(env,'subscriptions','user_id')))await env.DB.prepare('ALTER TABLE subscriptions ADD COLUMN user_id TEXT').run();
  if(!(await hasColumn(env,'items','user_id')))await env.DB.prepare('ALTER TABLE items ADD COLUMN user_id TEXT').run();
  await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_subscriptions_user ON subscriptions(user_id)').run();
  await env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_items_user ON items(user_id)').run();
...
async function getOrCreateLink(req,env){
  ...
  return json({ok:true,endpoint:url.origin+'/api/command?key='+encodeURIComponent(key)});
}
async function commandUser(req,env){
  await ensureSchema(env);
  const key=new URL(req.url).searchParams.get('key')||'';if(!key)return null;
  const row=await env.DB.prepare('SELECT uid FROM shortcut_links WHERE link_key=?').bind(key).first();
  return row&&row.uid?{uid:row.uid}:null;
}
...
async function readCommandText(req){
  const url=new URL(req.url),q=url.searchParams.get('text');
...
async function ingestCommand(req,env){
  const user=await commandUser(req,env);if(!user)return json({ok:false,error:'unauthorized'},401);
  ...
  await env.DB.prepare('INSERT INTO items (id,user_id,text,type,title,event_at,created_at,source) VALUES (?,?,?,?,?,?,?,?)')
    .bind(id,user.uid,text,parsed.type,parsed.title,parsed.eventAt,now,'siri').run();
  if(parsed.eventAt&&parsed.reminders.length)for(const offset of parsed.reminders){
    const notifyAt=parsed.eventAt-offset*60000;
    if(notifyAt>now-5*60000)await env.DB.prepare('INSERT INTO reminders (id,item_id,notify_at,offset_minutes,sent) VALUES (?,?,?,?,0)')
...
async function listItems(req,env){
  const user=await verifyFirebaseUser(req);if(!user)return json({ok:false,error:'unauthorized'},401);
  await ensureSchema(env);
  const rows=await env.DB.prepare('SELECT i.id,i.text,i.type,i.title,i.event_at,i.created_at,GROUP_CONCAT(r.offset_minutes) AS offsets FROM items i LEFT JOIN reminders r ON r.item_id=i.id WHERE i.user_id=? GROUP BY i.id ORDER BY i.created_at ASC').bind(user.uid).all();
...
async function syncItems(req,env){
  const user=await verifyFirebaseUser(req);if(!user)return json({ok:false,error:'unauthorized'},401);
  await ensureSchema(env);
  const body=await req.json().catch(()=>null),incoming=Array.isArray(body&&body.items)?body.items.filter(Boolean):[],now=Date.now();
  const ids=[];
  for(const x of incoming){
    const id=String(x.id||randomId('item'));ids.push(id);
    const eventAt=toOmanEpoch(String(x.date||''),String(x.time||''));
    await env.DB.prepare('INSERT INTO items (id,user_id,text,type,title,event_at,created_at,source) VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET user_id=excluded.user_id,text=excluded.text,type=excluded.type,title=excluded.title,event_at=excluded.event_at,source=excluded.source')
      .bind(id,user.uid,String(x.title||''),String(x.type||'task'),String(x.title||''),eventAt,Date.parse(x.created||'')||now,String(x.source||'app')).run();
    await env.DB.prepare('DELETE FROM reminders WHERE item_id=? AND sent=0').bind(id).run();
    if(!x.done&&eventAt&&Array.isArray(x.reminders))for(const off0 of x.reminders){
      const off=Number(off0);if(!Number.isFinite(off))continue;
      const notifyAt=eventAt-off*60000;
      if(notifyAt>now-5*60000)await env.DB.prepare('INSERT INTO reminders (id,item_id,notify_at,offset_minutes,sent) VALUES (?,?,?,?,0)').bind(randomId('rem'),id,notifyAt,off).run();
    }
  }
  const existing=await env.DB.prepare('SELECT id FROM items WHERE user_id=?').bind(user.uid).all();
  const keep=new Set(ids);
  for(const row of existing.results||[])if(!keep.has(String(row.id))){
    await env.DB.prepare('DELETE FROM reminders WHERE item_id=?').bind(row.id).run();
    await env.DB.prepare('DELETE FROM items WHERE id=? AND user_id=?').bind(row.id,user.uid).run();
  }
...
async function subscribe(req,env){
  ...
  const now=Date.now();
  await env.DB.prepare('INSERT INTO subscriptions(endpoint,p256dh,auth,user_id,created_at,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(endpoint) DO UPDATE SET p256dh=excluded.p256dh,auth=excluded.auth,user_id=excluded.user_id,updated_at=excluded.updated_at')
    .bind(s.endpoint,s.keys.p256dh,s.keys.auth,user.uid,now,now).run();
  return json({ok:true});
}
async function sendPushToUser(env,uid,payload){
  configureVapid(env);await ensureSchema(env);
  const subs=await env.DB.prepare('SELECT endpoint,p256dh,auth FROM subscriptions WHERE user_id=?').bind(uid).all();
  let ok=0,fail=0;
  for(const s of subs.results||[]){
    try{
      ...
      fail++;const status=e?.statusCode||0;
...
async function testMe(req,env){
  const user=await verifyFirebaseUser(req);if(!user)return json({ok:false,error:'unauthorized'},401);
  return json({ok:true,result:await sendPushToUser(env,user.uid,{title:'مُعين',body:'تم ربط الإشعارات الخارجية بنجاح. يمكنك إغلاق التطبيق.',tag:'mueen-server-test',url:'https://turkik69.github.io/Mueen/'})});
}
async function processDue(env){
  await ensureSchema(env);const now=Date.now();
  const due=await env.DB.prepare('SELECT r.id,r.item_id,r.offset_minutes,i.title,i.user_id FROM reminders r JOIN items i ON i.id=r.item_id WHERE r.sent=0 AND r.notify_at<=? AND i.user_id IS NOT NULL ORDER BY r.notify_at ASC LIMIT 50').bind(now).all();
  for(const r of due.results||[]){
    try{
      const result=await sendPushToUser(env,r.user_id,{title:'مُعين',body:reminderBody({title:r.title},Number(r.offset_minutes)||0),itemId:r.item_id,tag:'mueen-'+r.id,url:'https://turkik69.github.io/Mueen/?item='+encodeURIComponent(r.item_id)});
...
export default {
  async fetch(req,env){
    ...
    else if(url.pathname==='/api/sync'&&req.method==='POST')res=await syncItems(req,env);
    else if(url.pathname==='/api/test-me'&&req.method==='POST')res=await testMe(req,env);
    else if(url.pathname==='/health')res=json({ok:true,service:'mueen-reminders',version:'3.5'});
